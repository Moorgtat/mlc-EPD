...
const actions = {
    async fetchIdentity ({ commit }) {
        const response = await axios.get('https://94.23.47.22:3002/api/acces/getIdentity');
        commit('setIdentity', response.data);
    },
    async updtIdentity({ commit }, updidentity){
        const response = await axios.put('https://94.23.47.22:3002/api/acces/putIdentity?id=' + updidentity.id +
