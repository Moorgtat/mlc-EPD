...
const actions = {
    async fetchProjekts ({ commit }) {
        const response = await axios.get('https://94.23.47.22:3002/api/acces/getProjekts');
        commit('setProjekts', response.data);
    },
    async addProjekt({ commit }, projekt){
        const response = await axios.post('https://94.23.47.22:3002/api/acces/newProjekt?titre=' + projekt.titre + '&description='
    ...
    async deleteProjekt({ commit }, id){
        await axios.get('https://94.23.47.22:3002/api/acces/deleteProjekt?id=' + id);
        commit('removeProjekt', id)
    },
    async updtProjekt({ commit }, updprojekt){
        const response = await axios.put('https://94.23.47.22:3002/api/acces/putProjekt?id=' + updprojekt.id +
