...
const actions = {
    async fetchContacts ({ commit }) {
        const response = await axios.get('http://94.23.47.22:3002/api/acces/getContacts');
        commit('setContact', response.data);
    },
    async addContact({ commit }, contact){
        const response = await axios.post('http://94.23.47.22:3002/api/acces/newContact?name=' + contact.name + '&mail='
            + contact.mail + '&message=' + contact.message);
        commit('newContact', response.data);
    },
    async deleteContact({ commit }, id){
        await axios.get('http://94.23.47.22:3002/api/acces/deleteContact?id=' + id);
